...
getHorarios();
    ...
    function getHorarios(){
        ...
        if(isset($id_professor)){
            ...
            $sql = "SELECT DISTINCT disciplina.abreviacao as abreviacao, DIASEMANA.horario_inicio, DIASEMANA.horario_fim, DIASEMANA.Dia_Semana, CURSO.nome_curso,AULASEMESTRAL.id_periodo, AULASEMESTRAL.turno, DISCIPLINA.EXTERNO FROM DIASEMANA INNER JOIN AULASEMESTRAL ON DiaSemana.id_aulasemestral = AULASEMESTRAL.id INNER JOIN Professor ON AULASEMESTRAL.id_professor = Professor.id_Professor INNER JOIN Disciplina ON AULASEMESTRAL.id_Disciplina = Disciplina.id_Disciplina INNER JOIN HORARIOSEXTERNOS ON Disciplina.externo = HORARIOSEXTERNOS.ID_HAE INNER JOIN CURSO ON AULASEMESTRAL.id_curso = CURSO.id_curso  WHERE AULASEMESTRAL.id_professor = ? AND ((SELECT EXTRACT(YEAR FROM AULASEMESTRAL.DATA_INICIO)) <= (SELECT EXTRACT(YEAR FROM ?)) AND (SELECT EXTRACT(YEAR FROM AULASEMESTRAL.DATA_FIM)) >= (SELECT EXTRACT(YEAR FROM ?))) AND (? >= AULASEMESTRAL.DATA_INICIO AND (? <= AULASEMESTRAL.DATA_FIM OR ? <= AULASEMESTRAL.DATA_FIM OR ? <= AULASEMESTRAL.DATA_FIM OR ? <= AULASEMESTRAL.DATA_FIM OR ? <= AULASEMESTRAL.DATA_FIM))";           $stmt = $con->prepare($sql);
