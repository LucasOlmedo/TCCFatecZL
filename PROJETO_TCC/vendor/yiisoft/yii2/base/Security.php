...
class Security extends Component
{
    ...
    public function generateRandomKey($length = 32)
    {
        ...
        if (!is_int($length)) {
            throw new InvalidParamException('First parameter ($length) deve ser um inteiro');
        }

        if ($length < 1) {
            throw new InvalidParamException('First parameter ($length) deve ser maior que 0');
        }
    ...
    public function validatePassword($password, $hash)
    {
        if (!is_string($password) || $password === '') {
            throw new InvalidParamException('A senha deve ser um string e não pode ser vazia.');
        }
        ...
        if (!preg_match('/^\$2[axy]\$(\d\d)\$[\.\/0-9A-Za-z]{22}/', $hash, $matches)
            ...
            throw new InvalidParamException('Hash é inválido.');
        }
