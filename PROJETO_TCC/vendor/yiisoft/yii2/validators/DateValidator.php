...
class DateValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = Yii::t('yii', 'O formato de {attribute} é inválido.');
        }
        if ($this->format === null) {
        ...
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = Yii::t('yii', '{attribute} não deve ser menor que {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = Yii::t('yii', '{attribute} não deve ser maior que {max}.');
        }
        ...
        if ($this->max !== null && is_string($this->max)) {
            ...
            if ($timestamp === false) {
                throw new InvalidConfigException("Valor máximo de data inválido: {$this->max}");
            }
            $this->max = $timestamp;
        }
        if ($this->min !== null && is_string($this->min)) {
            ...
            if ($timestamp === false) {
                throw new InvalidConfigException("Valor mínimo de data inválido: {$this->min}");
            }
            $this->min = $timestamp;
        }
    }
